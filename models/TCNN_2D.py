...
class TCNN_2D(nn.Module):
    def __init__(self, input_dim, output_dim, win_size, imu_dim, n_filters=64, kernel_size=5, n_convolutions=2, dim_fc = 128, dropout=0.1, pooling_layer='None'):        
        ...
        for _ in range(self.n_convolutions):
            ...
            conv_layer = nn.Sequential(nn.Conv2d(d_in, self.n_filters, kernel_size=(1, self.kernel_size)), nn.ReLU())
            self.convolutions.append(conv_layer)
        
        self.dropout = nn.Dropout(dropout)
        
        if self.use_pooling:
            pooling_kernel = (self.n_devices, 1)
            self.pooling_layer = nn.AvgPool2d(pooling_kernel) if pooling_layer=='avg' else nn.MaxPool2d(pooling_kernel)

        # Computing input_dim for the first fc-layer = filters * devices * win_size (after convolutions)
        ...
        self.fc2 = nn.Linear(self.dim_fc, output_dim)
    ...
    def forward(self, data):
        ...
        # Applying convolutions: [B, IMU_Dim, N_IMU, Win] -> [B, N_Filter, N_IMU, Win']
        ...
        if self.use_pooling:
            # [B, N_Filter, N_IMU, Win'] -> [B, N_Filter, 1, Win']
            target = self.pooling_layer(target)
        
        # Flattening the tensor to [B, _] for final fully connected layers.
